/**
 * Generic UI audio helper retained for future Enqueteur UX layers.
 *
 * Intentionally asset-agnostic: callers provide URLs at runtime.
 */
    ...
    return {
        ...
        async load(map: Partial<Record<UiSfxKey, string>>): Promise<void> {
            ...
            for (const [key, url] of entries) {
                ...
                const audio = ensureAudioForKey(key, url, {
                    eagerLoad: true,
                    onError: (error) => {
                        logDevOnce(`load:${key}:${url}`, `[webview][ui-audio] failed to load '${key}' from ${url}.`, error);
                    },
                });
                audio.volume = volume;
            }
        ...
        play(key: UiSfxKey): void {
            ...
            const audio =
                audioByKey.get(key) ??
                (url
                    ? ensureAudioForKey(key, url, {
                          eagerLoad: false,
                          onError: (error) => {
                              logDevOnce(`play-load:${key}:${url}`, `[webview][ui-audio] could not initialize '${key}' from ${url}.`, error);
                          },
                      })
                    : undefined);
            const now = Date.now();
            ...
            void audio.play().catch((error) => {
                logDevOnce(`play:${key}`, `[webview][ui-audio] playback unavailable for '${key}'.`, error);
                playFallbackTone(key);
            });
        },
    };
