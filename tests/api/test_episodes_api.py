...
def test_get_latest_episode_ok(monkeypatch):
    ...
    # Provide a stable analysis result for the latest record
    from loopforge.analytics.reporting import DaySummary, EpisodeSummary, AgentDayStats, AgentEpisodeStats
    ...
    def fake_analyze_episode_from_record(record, *, action_log_path, supervisor_log_path=None):
        ...
        day0 = DaySummary(day_index=0, perception_mode="accurate", tension_score=0.5, agent_stats=agent_stats, total_incidents=0, supervisor_activity=0.0)
        agents = {
            "Alpha": AgentEpisodeStats(
                name="Alpha", role="scout", guardrail_total=1, context_total=2, trait_deltas={}, stress_start=0.2, stress_end=0.3, representative_reflection=None, visual="", vibe="", tagline="", trait_snapshot=None
            )
        }
        ep = EpisodeSummary(days=[day0], agents=agents, tension_trend=[0.5], episode_id=record.episode_id, run_id=record.run_id, episode_index=record.episode_index)
        return ep

    import loopforge.analytics.analysis_api as analysis_api
    monkeypatch.setattr(analysis_api, "analyze_episode_from_record", fake_analyze_episode_from_record)

    # Stabilize narrative helpers for determinism
    import loopforge.stage.builder as builder_mod

    monkeypatch.setattr(
        builder_mod,
        "build_day_narrative",
        lambda day_summary, day_index, previous_day_summary=None: SimpleNamespace(
            agent_beats=[SimpleNamespace(name="Alpha", intro="I", perception_line="see", actions_line="do", closing_line="end")],
            day_intro=f"Day {day_index} intro",
            supervisor_line="Supervisor speaks",
            day_outro=f"Day {day_index} outro",
    ...
    monkeypatch.setattr(
        builder_mod,
        "build_episode_recap",
        lambda episode_summary, day_summaries, character_defs: SimpleNamespace(intro="Recap intro", per_agent_blurbs={"Alpha": "Alpha blurb"}, closing="Recap end"),
    )
...
def test_get_latest_episode_no_episodes_404(monkeypatch):
    import loopforge.analytics.run_registry as rr
    monkeypatch.setattr(rr, "load_registry", lambda base_dir=None: [])

    client = TestClient(api_app.app)
    resp = client.get("/episodes/latest")
    assert resp.status_code == 404
    assert resp.json().get("detail") == "No episodes found"
