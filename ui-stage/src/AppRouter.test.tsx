...
describe("AppRouter routes", () => {
    ...
    it("main nav exposes Stage and Details links", () => {
        ...
        const navs = within(container).getAllByRole("navigation", {
    ...
    // Helper scoped to each render to avoid interference from other tests / StrictMode dual renders
    function getNavTripletFrom(container: HTMLElement) {
        const navs = within(container).getAllByRole("navigation", {
            name: /Main navigation/i,
        });
        const nav = navs[navs.length - 1];
    ...
    it("nav highlighting — Stage active at root /", async () => {
        ...
        await waitFor(() => {
            const { stageLink, detailsLink, episodesLink } =
                getNavTripletFrom(container);
