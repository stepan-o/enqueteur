...
LLM / Semantic Sidecar, Substrate Interface & Tick Hooks
...
This is the single source of truth for the interface between narrative and the Sim4 engine core.
...
Builds a NarrativeTickContext (see §5.1).
...
Runtime must not apply any ECS/world mutations from this call directly.
Instead:
...
diff_summary: dict                        # compact summary of last tick diffs
...
narrative/ folder exists with at least:
...
NarrativeTickContext, NarrativeTickOutput, SubstrateSuggestion, StoryFragment, and MemoryUpdate:
