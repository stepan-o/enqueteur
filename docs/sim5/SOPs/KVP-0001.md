...
>
> MUST rules:
> - Sprint 14 Offline Artifact Replay MUST NOT send or require any `REPLAY_*` messages.
> - `REPLAY_*` is only valid on LIVE transports that support kernel-hosted replay.
>
> Interop notes:
> - State records in artifacts REUSE the same envelope + `msg_type` + payload schemas as LIVE state delivery.
> - Artifact organization (chunking/indexing/paths/URLs) is out-of-scope here; see the Export Appendix for layout and naming.
...
C.2 Envelope.payload is intentionally serde_json::Value to enforce envelope-first dispatch; typed payload structs are decoded after matching msg_type.
