...
- REPLAY diff semantics are explicit: `to_tick = from_tick + 1` (no chunking in v0.1). Diffs MUST be applied in ascending tick order. If chunking is introduced in a future version, it will use a new message type (e.g., `DIFF_CHUNK`) with explicit semantics; v0.1 does not allow implicit chunking.
...
- Removal deletes entity by ID (rooms by room_id; agents/items by ID; events by (tick,event_id); narrative by (entity_id, kind, text)).
  - Narrative removal identity (entity_id, kind, text) is deterministic, but risky for size - it can be heavy if text is long.
  - v0.1 uses text-identity; v0.2 may introduce fragment_id to avoid large removal keys.
- If a full replace behavior is used anywhere, it MUST be named `*_full_replace` (not used in v0.1 by default).

Identity clarification (v0.1):
- Narrative removal identity is exactly `(entity_id, kind, text)`. Removals MUST match the exact text. No additional identifiers are introduced in v0.1.

Narrative text stability (v0.1):
- `text` MUST be byte-for-byte stable for the lifetime of the fragment. Producers MUST NOT mutate a fragment's `text` in place. Any change to `text` MUST be modeled as a removal of the old fragment followed by an upsert of a new fragment. This rule is required for deterministic identity and safe removals.
...
In v0.1, kernels MAY stream replay snapshots/diffs over the LIVE transport, but viewers MUST support ARTIFACTS delivery; ARTIFACTS is the portable default.
...
**Kernel responses**
- `DESYNC_CONFIRMED` MUST include an authoritative `recovery_tick`. Kernel MUST then send a `FULL_SNAPSHOT` for `recovery_tick`. Streaming of `FRAME_DIFF` MUST resume only AFTER that snapshot is applied. Viewer MUST perform a hard reset from the snapshot (forbid partial merges).
- or `DESYNC_DENIED`

Desync recovery is protocol-defined, not viewer-defined. Viewer MUST discard divergent local state and rebuild from the provided `FULL_SNAPSHOT(recovery_tick)`, then apply subsequent `FRAME_DIFF` in order.
...
impl Subscribe {
    ...
    pub fn validate_v0_1(&self) -> Result<(), &'static str> {
        match self.stream {
            StreamMode::Live => Ok(()),
            _ => Err("StreamMode=REPLAY is reserved in KVP-0001 v0.1; SUBSCRIBE.stream must be LIVE"),
        }
    }
}
