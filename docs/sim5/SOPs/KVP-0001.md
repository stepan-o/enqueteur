...
- MUST preserve in-order delivery; loss/failure MUST be surfaced and recovered via resubscribe + snapshot.
...
- Payload objects carry no inner type discriminator in v0.1.
  - The “no inner discriminator” rule applies to message payloads, but nested objects may be tagged (like cmd.type).
- Decoders MUST switch on envelope.msg_type to decode payload; do not rely on an inner tag.
...
Apply semantics (v0.1 Fix Pack):
- Upsert replaces entity by ID if present, otherwise inserts (rooms by room_id; agents/items by their IDs; events by (tick,event_id); narrative by (entity_id, kind, text)).
- Removal deletes entity by ID (rooms by room_id; agents/items by ID; events by (tick,event_id); narrative by (entity_id, kind, text)).
- If a full replace behavior is used anywhere, it MUST be named `*_full_replace` (not used in v0.1 by default).
...
Viewer MAY include an opaque `viewer_state_token` for debugging; it is not required to be comparable.
...
pub struct DiffBody {
    ...
    // v0.1 Fix Pack semantics: explicit upserts + removals
    pub rooms_upserts: Vec<RoomSnapshot>,
    pub rooms_removals: Vec<i32>, // room_id list

    pub events_upserts: Vec<EventSnapshot>,
    pub events_removals: Vec<EventKey>,

    pub narrative_upserts: Vec<NarrativeFragment>,
    pub narrative_removals: Vec<NarrativeRemovalKey>,
}
...
impl Canonicalize for DiffBody {
    fn canonicalize(&mut self) {
        ...
        sort_by_key_stable(&mut self.rooms_upserts, |r| r.room_id);
        for r in &mut self.rooms_upserts {
            r.canonicalize();
        }

        self.rooms_removals.sort();

        self.events_upserts.sort_by_key(|e| (e.tick, e.event_id));
        for e in &mut self.events_upserts {
            e.canonicalize();
        }

        self.events_removals
            .sort_by_key(|k| (k.tick, k.event_id));

        self.narrative_upserts
            .sort_by(|a, b| (a.entity_id, a.kind, &a.text).cmp(&(b.entity_id, b.kind, &b.text)));
        for n in &mut self.narrative_upserts {
            n.canonicalize();
        }

        self.narrative_removals
            .sort_by(|a, b| (a.entity_id, a.kind, &a.text).cmp(&(b.entity_id, b.kind, &b.text)));
    }
}
...
pub struct NarrativeRemovalKey {
    pub entity_id: i32,
    pub kind: NarrativeKind,
    pub text: String,
}
